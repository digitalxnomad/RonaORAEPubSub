...
- ✨ Reported by RONA QA; only the `ea_sale` baseline moved, by that one field.

### v1.0.106 (08/18/26)
**Version marker — no mapping changes.**
- ℹ️ **No behaviour change.** Output is byte-for-byte identical to v1.0.105 for every sample; all 141 regression tests pass unchanged. This bump marks a build, it does not carry a fix.
